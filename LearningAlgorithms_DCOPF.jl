...
function RunLearningAlgorithm(alpha, delta, epsilon, gamma, scenarios, testsize, Mmin)
    ...
    R_max = StoppingCriterion(alpha, epsilon)
...
function RunStreamingAlgorithm_Convergence(alpha, delta, epsilon, gamma, scenarios, testsize, Mmin)

    iteration_M = Int[]
    iteration_W = Int[]
    iteration_K_M = Int[]
    iteration_R_MW = Float64[]
    ...
    push!(iteration_W, W)
    push!(iteration_M, m)
    push!(iteration_K_M, K_M)
    push!(iteration_R_MW, R_MW)
    ...
    if R_MW <=R_max
        ...
        return m, W, R_MW, K_M, R_OS, testsize, iteration_W, iteration_M, iteration_K_M, iteration_R_MW
    end
