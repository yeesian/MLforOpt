...
using JLD, OPFRecourse, MAT, Plots
...
iteration_W = Vector{Vector{Int}}(15)
iteration_M = Vector{Vector{Int}}(15)
iteration_K_M = Vector{Vector{Int}}(15)
iteration_R_MW = Vector{Vector{Float64}}(15)
#K_M = Dict{String,Any}()
#W = Dict{String,Any}()
...
#])
    ...
    #data_file = string("DCOPF Darwin Results/results_uniform/", f, "3.jld")
    data_file = string("DCOPF Darwin Results/results_gaussian/", f, "3.jld")
    ...
    (M[k], W[k], R_MW[k], K_M[k], R_OS[k], W_test[k], iteration_W[k], iteration_M[k], iteration_K_M[k], iteration_R_MW[k]) = RunStreamingAlgorithm_Convergence(alpha, delta, epsilon, gamma, scenarios, testsize, Mmin)
...
plot(count, [nKM, nRMW], layout=(2,1))
